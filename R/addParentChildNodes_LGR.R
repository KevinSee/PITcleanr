#' @title Add Nodes to Parent-Child Table - LGR
#'
...
#' child locations now refer to nodes, not sites. This function is specific to Lower Granite Dam
#' and the Snake River; for other cases see `addParentChildNodes()`.
#'
#' @author Kevin See and Ryan Kinzer
#'
...
addParentChildNodes_LGR = function(parent_child = NULL,
  ...
  node_long = tibble(site_code = union(parent_child$child,
    ...
    left_join(configuration %>%
                select(site_code, node) %>% # included site code and removed the mutate section to develop site_code
                distinct(),
              by = "site_code") %>%
    distinct() %>%
    arrange(site_code, node) %>% # remove desc...
    group_by(site_code) %>%
  ...
  node_long <- node_long %>%
    mutate(node = ifelse(is.na(node), site_code, node))
