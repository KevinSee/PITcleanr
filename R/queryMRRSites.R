...
queryMRRSites = function(site = NULL) {
  ...
  parsed = parsed %>%
    purrr::map(.f = function(x) {
      for(i in 1:length(x)) {
        if(is.null(x[[i]])) {
          # if(names(x)[i] %in% c('facId', 'lineId', 'polyId')) {
          #   x[[i]] = vector('integer', 1)
          #   x[[i]] = NA
          # }
          if(names(x)[[i]] %in% c('isMarkSiteOnly', 'pointLocation')) {
    ...
    purrr::map_df(`[`,
                  c('siteCode', 'siteName', 'rkmMask', 'siteType', 'siteTypeDescription', 'huC4Code', 'latitude', 'longitude', 'coordinateType', 'minKm', 'maxKm', 'isMarkSiteOnly', 'changedBy', 'changeComments', 'changeDate', 'pointLocation', 'shapeType', 'locationId')) %>%
