...
Not all PIT tag data is in PTAGIS.This vignette will show the user how to read in PIT tag data from a variety of other sources, including Biologic csv files as well as raw files (e.g. .xlsx, .log and .txt file extensions) downloaded directly from PIT tag readers. The `r readCTH()` function is able to read in all of these formats, but the user must indicate what format each file is in. 
...
We set the name and path of the folder with all the detection data (`r detection_folder`), and used the following script to compile a data.frame of various file names and file types. A user can set the `r detection_folder` path to point to where they have stored all their detection files.
...
The following script uses the path of the `r detection_folder`, the various file names inside that folder, the file type associated with each file, and the `r readCTH()` function to read all the detections into R and consolidate them into a single data.frame, `r all_obs`. It contains a quick check (`r try` and `r "try-error"`) to ensure that if any particular file has trouble being read, the others are still included. 
```{r}
# read them all in
all_obs <-
  file_df |> 
  mutate(obs_df = map2(file_nm,
                       file_type,
                       .f = function(x, y) {
                         try(readCTH(paste0(detection_folder, x), y))
                       })) |> 
