...
Directions on how to install `PITcleanr` can found at its GitHub homepage [here](https://github.com/BiomarkABS/PITcleanr). It should be noted that many of the various queries in `PITcleanr` require a connection to the internet. If the user is somewhat familiar with installing R packages from GitHub, the following code can be used:
...
The Columbia Basin PIT Tag Information System ([PTAGIS](https://ptagis.org/)) is the centralized regional database for PIT-tag detections within the Columbia Basin. It contains a record of each detection of every PIT tag, including the initial detection, or "mark", when the tag is embedded in the fish, detections on PIT-tag antennas, recaptures (e.g. at weirs) and recoveries (e.g. carcass surveys). It contains a record of every individual detection, which means potentially multiple records of a tag being detected on the same antenna over and over if it is not moving. Querying PTAGIS for all of these detections leads to a wealth of data, but it can be unwieldy for the user. `PITcleanr` aims to compress that data to a more manageable size, without losing any of the information contained in that dataset. 

`PITcleanr` starts with a complete capture history query from [PTAGIS](https://ptagis.org/) for a select group of tags of interest. The user will need to compile this list of tags themselves, ideally in a .txt file with one row per tag number, to make it easy to upload to a PTAGIS query. For an example of one such file, the following code can be used to find the example file included with `PITcleanr`.
...
Set up a filter for specific tags by next navigating to the "29 Tag Code - List or Text File" on the left. And then, after selecting "Tag" under "Attributes:", click on "Import file...". Simply upload the .txt file prepared earlier containing your PIT tag codes of interest. Under "Report Message Name:" near the bottom, name the query something appropriate, such as "TUM_Chinook_2015.csv", and select "Run Report". Once the query has successfully completed, export the output as a .csv file (e.g. "TUM_Chinook_2015.csv") using the default settings:
...
We have included an example of data downloaded from PTAGIS with the `PITcleanr` package. The user can find where that example file has been saved by running the following code. 
```{r}
...
ptagis_file = "C:/Users/Downloads/TUM_Chinook_2015.csv"
```
...
The complete tag history query output from [PTAGIS](https://ptagis.org/) will provide a record for every detection of each tag code in the tag list. This may include multiple detections on the same antenna, or same site within a short period of time. One of the main purposes of `PITcleanr` is to compress that data into an initial summary using the function `compress()`. At a minimum. this function requires a path to the complete tag history file downloaded from PTAGIS, as seen at the end of the last section. 

Then the `compress()` function can be performed on the file path. In our example, we write the output to an object `comp_obs` containing the compressed observations.
...
Note that we have scaled down the raw data from PTAGIS from `r nrow(raw_ptagis)` rows of data to `r nrow(comp_obs)` rows. At the same time, the sum of the `n_dets` column in `comp_obs` is `r sum(comp_obs$n_dets)`, showing that every detection from the raw data has been accounted for. For the rest of this vignette, we will be exploring other functionality of `PITcleanr` as it related to this compressed data.
