...
Later, we'll demonstrate how `PITcleanr` can be used to compress the detections in your complete tag history output (`ptagis_file`) from [PTAGIS](https:://www.ptagis/org). Bur first, `PITcleanr` can help perform some basic quality control on the PTAGIS detections. The `qcTagHistory()` function returns a list including a vector of tags listed as "DISOWN" by PTAGIS, another vector or tags listed as "ORPHAN" (which may indicate a mark file has not been uploaded), and a data frame of release batches. This latter data frame can be used by the `compress()` function in `PITcleanr` to determine whether to use the event time or the event release time as reported by PTAGIS for a particular site/year/species combination.
