...
* [Reading in data](read_in_data.Rmd)
* [Configuration files](configuration_files.Rmd)
* [Compressing data](compress_data.Rmd)
* [Parent-Child Tables](parent_child.Rmd)
...
```{r child = "_child_docs/install-inst.Rmd"}
```
...
In this vignette, we will use an example dataset of Spring Chinook Salmon tagged as adults at Tumwater dam in 2015. `PITcleanr` includes the [complete tag history](read_in_data.Rmd) downloaded from PTAGIS, the [configuration file](configuration_files.Rmd) and the [parent-child table](parent_child.Rmd) for this particular analysis. However, if the user needs to consult how to query or construct any of those, please read the appropriate vignette.
...
The first step is to [compress](compress_data.Rmd) the raw detection data. This maps each detection on an individual antenna to a node, defined in the `configuration` file, and combines detections on the same node to a single row. 
...
Note that in `PITcleanr`'s default settings, nodes that end in `B0` are generally the downstream array, while nodes that end in `A0` are the upstream array. For more information about that, see the [configuration files](configuration_files.Rmd) vignette. 
