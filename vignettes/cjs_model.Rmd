...
output: 
  rmarkdown::html_vignette:
    ...
    toc: true
vignette: >
  %\VignetteIndexEntry{Preparing Data for a Cormack-Jolly-Seber (CJS) model}
...
In our example, here are the capture histories of the `r n_distinct(prepped_df$tag_code[is.na(prepped_df$user_keep_obs)])` tags with non-straightforward movements, along with `PITcleanr`'s suggestions as to which detections to keep (`auto_keep_obs == TRUE`) and which ones to discard (`auto_keep_obs == FALSE`). 
...
column_order <- c("B2J", "JDJ", "MCJ", "ICH", "LMJ", "GOJ", "GRJ", "LLR", "LLRTP", "EVL", "EVU", "LEMTRP")
...
To demonstrate how a CJS model might get more nuanced, we will examine whether survival and detection at each site depend on whether a fish is tagged at the Upper Lemhi screw trap in the fall as a parr, or in the spring as a smolt. First we assign a life-stage to each tag, and prepare the data again.
...
# add life history type to capture histories
cjs_data <-
  cap_hist |> 
  left_join(covar_df,
            by = join_by(tag_code)) %>%
...
The `marked` package allows a user to fit a number of different user-defined models to the same dataset, and returns some model selection information such as AIC. 

```{r, eval = F}
#------------------------
# set up possible models

# survival and detection only depend on location
Phi.time <- list(formula = ~ time)
p.time <- list(formula = ~ time)

# survival and detection depend on location and life-stage
Phi.time.grp <- list(formula = ~ time * life_stage)
p.time.grp <- list(formula = ~ time * life_stage)

# create all combinations of Phi and p models
...
```{r fit-ls-cjs-mod, results = "hide", echo = F}
# survival and detection depend on location and life-stage
Phi.time.grp <- list(formula = ~ time * life_stage)
p.time.grp <- list(formula = ~ time * life_stage)

# fit full model
mod_ls <- crm(data = cjs_proc,
              ddl = cjs_ddl,
              model.parameters = list(Phi = Phi.time.grp,
                                      p = p.time.grp),
              hessian = T)
```

In this case, the model best supported by AIC included different survival and detection probabilities for each life stage, parr and smolt. We can extract those estimates and assign a reach to each survival parameter and site to each detection parameter. 
...
We can then plot the estimates, for both the model separating parameters by life-stage, and the model which combines all that data. 

```{r surv-est}
pd = 0.2

est_preds_ls$Phi |> 
  bind_rows(est_preds$Phi |>
              mutate(life_stage = "All")) |>
  mutate(across(life_stage,
                ~ str_to_title(.))) |> 
  filter(occ < max(occ)) |> 
  mutate(across(reach,
                forcats::as_factor)) |> 
